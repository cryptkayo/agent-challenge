...
async function webSearch(query: string): Promise<string> {
  ...
  // Use Serper.dev if available (free tier: 2500 queries/mo)
  ...
  return abstract
    ? `${abstract}\n\nRelated:\n${relatedTopics}`
    : `Search returned limited results for: "${query}". Related:\n${relatedTopics}`;
}

// ─── TOOL: Research Narrative ─────────────────────────────────────────────────
export const researchNarrativeTool = createTool({
  id: "researchNarrativeTool",
  description:
    "Research trending crypto narratives, market themes, or specific topics. Returns a structured brief with signal separated from noise.",
...
// ─── TOOL: Watchlist Check ────────────────────────────────────────────────────
export const watchlistCheckTool = createTool({
  id: "watchlistCheckTool",
  description:
    "Cross-reference the user's personal watchlist against current crypto news. Returns only relevant findings.",
  inputSchema: z.object({
  ...
  execute: async ({ context }) => {
    const watchlist = readJSON<string[]>("watchlist.json", []);
    ...
    const findings = await Promise.all(
      watchlist.slice(0, 5).map(async (asset) => {
        const query = context.focusArea
          ? `${asset} ${context.focusArea} crypto news`
          : `${asset} crypto news today`;
        ...
        return {
          ...
          sentiment: hasBullish ? ("bullish" as const) : hasBearish ? ("bearish" as const) : ("neutral" as const),
        };
      })
    );
...
// ─── TOOL: Draft Thread ───────────────────────────────────────────────────────
export const draftThreadTool = createTool({
  id: "draftThreadTool",
  description:
    "Draft a Twitter/X thread based on research findings. Returns numbered tweets ready to post.",
  inputSchema: z.object({
  ...
  execute: async ({ context }) => {
    ...
    // Build thread structure based on tone
    const toneGuide = {
    ...
    // Generate placeholder tweets (real generation happens via the LLM agent)
    const tweets = Array.from({ length: tweetCount }, (_, i) => {
      const texts: Record<number, string> = {
        0: `🧵 Thread on ${topic}: Here's what you need to know right now (${i + 1}/${tweetCount})`,
        ...
        4: `Bottom line: ${topic} is one of the most important narratives right now. Position accordingly. DYOR 🫡`,
      };
...
// ─── TOOL: Daily Brief ────────────────────────────────────────────────────────
export const dailyBriefTool = createTool({
  id: "dailyBriefTool",
  description:
    "Compile a full morning briefing: top narratives + watchlist check + pending reminders. One command to know everything.",
  inputSchema: z.object({
  ...
  execute: async ({ context }) => {
    ...
    const reminders = readJSON<Array<{ text: string; dueDate: string }>>("reminders.json", []);
    const watchlist = readJSON<string[]>("watchlist.json", []);
    ...
    const marketSentiment =
      bullCount > bearCount ? "risk-on" : bearCount > bullCount ? "risk-off" : "neutral";
...
// ─── TOOL: Save to Watchlist ──────────────────────────────────────────────────
export const saveToWatchlistTool = createTool({
  ...
  execute: async ({ context }) => {
    const current = readJSON<string[]>("watchlist.json", []);
...
// ─── TOOL: Save Reminder ──────────────────────────────────────────────────────
export const saveReminderTool = createTool({
  id: "saveReminderTool",
  ...
  execute: async ({ context }) => {
    const reminders = readJSON<Array<{ text: string; dueDate: string; createdAt: string }>>(
      "reminders.json",
      []
    );
    const newReminder = {
